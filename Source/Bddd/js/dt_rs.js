...
//请求数据
//从后台获取gps数据
var dataurl = "./data.ashx?xcout=" + colcount + "&ycout=" + rowcount + "&startx=" + startx + "&endx=" + endx + "&starty=" + starty + "&endy=" + endy + "&t=allzone";
$.ajax({
    type: "get",
    ...
    success: function (result) {     //回调函数，result，返回值
        if (result.length > 0) {
            var jsgrid = $("#mapDiv").jsGrid({
                ...
                fields: [
            { name: "orderid", title: "订单号", type: "text", validate: "required", width: 160 },
            { name: "StartTime", title: "开始时间", type: "date" },
            { name: "EndTime", title: "结束时间", type: "date" },
            { name: "startrectid", title: "开始小区", type: "text" },
            { name: "endrectid", title: "结束小区", type: "text" }
                ]
            });
